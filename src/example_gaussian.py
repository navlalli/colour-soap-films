...
def gaussian_thickness_colour(name="", save=0):
    ...
    of incidence. N = 5 used in the interference calculations.
...
if __name__ == "__main__":
    ...
    # Define the source spectral distribution
    shape = colour.SpectralShape(360, 830, 1)
    wavelengths = shape.wavelengths
    nw = len(wavelengths)
    mean_w = 550  # Mean wavelength (nm)
    ...
    nfilm = np.linspace(1.42, 1.40, nw)
    ...
    # Define source composed of the sum of Gaussians
    source_sd = define_another_source(0.015, name="double_gaussian_440_550nm", save=0)
    # Find variation of colour with thickness
